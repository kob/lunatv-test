...
function LivePageClient() {
  ...
  const cleanupPlayer = () => {
    ...
    if (shakaPlayerRef.current) {
      try {
        ...
        console.warn('清理 Shaka Player 时出错:', err);
        shakaPlayerRef.current = null;
      }
    }
  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      if (!videoUrl || !videoRef.current || !currentChannel) {
        return;
      }
      ...
      try {
        ...
        if (!shaka.Player.isBrowserSupported()) {
          console.error('当前浏览器不支持 Shaka Player');
          setUnsupportedType('browser-not-supported');
          setIsVideoLoading(false);
          return;
        }
        ...
        const config = {
          streaming: {
            ...
            ignoreAudioStreamFailures: true,
            failureCallback: (error: shaka.util.Error) => {
              console.error('Shaka Player streaming error:', error);
        ...
        console.log('✓ Shaka Player 初始化成功');
        setIsVideoLoading(false);
        setUnsupportedType(null);

        // 监听错误
        player.addEventListener('error', (event: shaka.util.Error) => {
          console.error('Shaka Player error:', event);
          handleShakaError(event);
        });
        ...
        if (videoRef.current) {
          ...
          videoRef.current.addEventListener('error', (event: Event) => {
        ...
        setTimeout(() => {
          if (videoRef.current && videoRef.current.seekable && videoRef.current.seekable.length > 0) {
            ...
            if (seekableRange > 60) {
              console.log('✓ 检测到支持 DVR，可拖动范围:', Math.floor(seekableRange), '秒');
              setDvrDetected(true);
        ...
        console.error('初始化 Shaka Player 失败:', err);
        setUnsupportedType('init-error');
        setIsVideoLoading(false);
      }
    };
    ...
    const handleShakaError = (error: shaka.util.Error) => {
      ...
      switch (error.code) {
        ...
        case shaka.util.Error.Code.LICENSE_RESPONSE_REJECTED:
          setUnsupportedType('drm-error');
          break;
        case shaka.util.Error.Code.NETWORK_ERROR:
          setUnsupportedType('network-error');
          break;
        case shaka.util.Error.Code.MIME_UNSUPPORTED:
        case shaka.util.Error.Code.MANIFEST_UNSUPPORTED_CODECS:
          setUnsupportedType('codec-incompatible');
          break;
        case shaka.util.Error.Code.REQUEST_FAILED:
          setUnsupportedType('request-failed');
          break;
        default:
          setUnsupportedType('playback-error');
      }
      setIsVideoLoading(false);
    };

    initPlayer();
  }, [videoUrl, currentChannel, loading, directPlaybackEnabled]);
    ...
    const handleKeyboardShortcuts = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowUp') {
        if (videoRef.current && videoRef.current.volume < 1) {
          ...
          console.log(`音量: ${Math.round(videoRef.current.volume * 100)}`);
          e.preventDefault();
        }
      }
